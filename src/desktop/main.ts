...
manager.add(['app.record.create.show', 'app.record.edit.show', 'app.record.index.edit.show'], (event) => {
  const config = restorePluginConfig();

  config.conditions.forEach(({ srcFieldCode, dstFieldCode }) => {
    if (!srcFieldCode || !dstFieldCode || !event.record[srcFieldCode] || !event.record[dstFieldCode]) {
      return;
    }
...
manager.add(['app.record.index.edit.submit', 'app.record.create.submit', 'app.record.edit.submit'], (event) => {
  const config = restorePluginConfig();
