export class KintoneEventManager {
  ...
  public addHeaderButton(params: {
    ...
    const { id, label, color = 'default', onClick, isButtonHidden, events, isTolerantToFailure = false } = params;
...
.rkemb[data-color="yellow"]:focus {
  ...
  private initialize = (event: kintoneAPI.js.Event) => {
    ...
    if (!this.#logDisabled) {
      console.group(`${this.#logPrefix}%c${event.type} %c(${this.#uid})`, 'color: #1e40af;', 'color: #aaa');
    }
  };
...
//     const { event } = props;
//     event.error = `プラグインの処理内でエラーが発生しました。`;
